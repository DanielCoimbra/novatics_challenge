...
describe('List Manipulation Test', () => {
    it('Should return the same list', () => {
        assert.deepEqual(f.remove_duplicate(list_ok), [1,2,3,4,5]);
    });
    it('Should return a list with a single item', () => {
        assert.deepEqual(f.remove_duplicate(list_only_1s), [1]);
    });
    it('Should return the list without the duplicates', () => {
        assert.deepEqual(f.remove_duplicate(list_doubled), [1,2,3,4,5]);
    });
    it('Should return an empty list', () => {
        assert.deepEqual(f.remove_duplicate(list_empty), []);
        });
});
